 ...
 *
 * 역할: 프런트엔드(GitHub Pages 정적 사이트)는 API 키를 가질 수 없다.
 * 이 Worker가 비밀(secret)로 보관된 GEMINI_API_KEY로 Google AI Studio
 * (Generative Language API)를 대신 호출하고, 교정된 Markdown만 돌려준다.
 *
 * 필요한 설정(Worker의 Variables & Secrets):
 *   - GEMINI_API_KEY  (필수, secret)  : https://aistudio.google.com 에서 발급
 *   - ALLOWED_ORIGIN  (선택, 기본 '*') : 허용할 사이트 origin
 *        예) https://chichiboo123.github.io
 *   - GEMINI_MODEL    (선택, 기본 'gemini-2.5-flash')
 *
 * 계약(contract):
 *   POST /  { text: string, lang?: 'ko'|'en'|'ja' }
